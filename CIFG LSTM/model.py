...
class LSTM:
    ...
    def softmax(self, x):
        ...
        """
        e_x = np.exp(
            x - np.max(x))  # max(x) subtracted for numerical stability
    ...
    def update_params(self, batch_num):
        ...
        for key in self.params:
            ...
            m_correlated = self.adam_params["m" +
                                            key] / (1 - self.beta1 ** batch_num)
            v_correlated = self.adam_params["v" +
                                            key] / (1 - self.beta2 ** batch_num)
            self.params[key] -= self.lr * m_correlated / \
                (np.sqrt(v_correlated) + 1e-8)
        return
    ...
    def forward_step(self, x, h_prev, c_prev):
        ...
        f = np.transpose(np.ones(self.i.shape[::-1]) - i.transpose())
    ...
    def gradient_check(self, x, y, h_prev, c_prev, num_checks=10, delta=1e-6):
        ...
        for key in self.params:
            ...
            print('Approximate: \t%e, Exact: \t%e =>  Error: \t%e' %
